...
init_error = None
try:
    # Check environment variables have a value assigned
    if not GOOGLE_API_KEY:
        init_error = 'Missing GOOGLE_API_KEY environment variable'
    elif not GOOGLE_AI_MODEL:
        init_error = 'Missing GOOGLE_AI_MODEL environment variable'

    if not init_error:
        gemini_ai.configure(api_key=GOOGLE_API_KEY)
        model = gemini_ai.GenerativeModel(GOOGLE_AI_MODEL)
...
def lambda_handler(event, context):
    ...
    # Check if there's an error during initialization
    if init_error:
        logger.error('Error configuring the generative AI model')
        return build_response(500, {"error": init_error})
